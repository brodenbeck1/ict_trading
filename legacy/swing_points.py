...
class SwingPointScanner:
    ...
    def get_recent_swings(self, n: int = 5, swing_type: Literal['high', 'low', 'both'] = 'both'):
        """
    ...
    def find_relative_equal_highs(self, tolerance: float = 0.0005):
        """
        ...
        Returns:
            List of tuples: (price_level, timestamps, count)
        """
    ...
    def find_relative_equal_lows(self, tolerance: float = 0.0005):
        """
...
if __name__ == "__main__":
    ...
    reader = csvReader(timeframe='15T', weeks=4, data_dir=data_dir)
    df = reader.read_NQ()
    ...
    print(f"\nBuy stops (above swing highs): {len(liquidity['buy_stops'])} levels")
    print(f"Sell stops (below swing lows): {len(liquidity['sell_stops'])} levels")
    ...
    for cluster in rel_equal_highs:
        print(f"  Price: {cluster['price']:.2f}, Count: {cluster['count']}")
